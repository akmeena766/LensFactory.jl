...
# Keyword Arguments
- `date::Union{Nothing, Date}=nothing`: Optional date to include in the FITS header
- `time::Union{Nothing, Time}=nothing`: Optional time to include in the FITS header
- `save_potential::Bool=true`: Whether to save the lensing potential as "potential.fits"
- `save_deflection::Bool=true`: Whether to save the deflection angles as "alpha_x.fits" and "alpha_y.fits"
- `save_kappa::Bool=true`: Whether to save the convergence map as "kappa.fits"
- `save_gamma::Bool=true`: Whether to save the shear maps as "gamma1.fits" and "gamma2.fits"
...
function get_potential(file_name::String, θx::T, θy::T; unit::Symbol=:RA_DEC) where T <: Union{ROA, Vector{Int64}}
   ...
   return get_potential(model, chains, chi2, θx, θy; unit=unit)
end

function get_potential(model::ModelConfig, chains::Array{Float64, 3}, chi2::Matrix{Float64}, θx::T, θy::T; unit::Symbol=:RA_DEC) where T <: Union{ROA, Vector{Int64}}
...
function get_deflection(file_name::String, θx::T, θy::T; unit::Symbol=:RA_DEC) where T <: Union{ROA, Vector{Int64}}
   ...
   return get_deflection(model, chains, chi2, θx, θy; unit=unit)
end

function get_deflection(model::ModelConfig, chains::Array{Float64, 3}, chi2::Matrix{Float64}, θx::T, θy::T; unit::Symbol=:RA_DEC) where T <: Union{ROA, Vector{Int64}}
   # Check if the input coordinates are of the same size
   if size(θx) != size(θy)
      throw(ArgumentError("Input coordinates must be of the same size."))
   end
...
function get_jacobian(file_name::String, θx::T, θy::T; unit::Symbol=:RA_DEC) where T <: Union{ROA, Vector{Int64}}
   ...
   return get_jacobian(model, chains, chi2, θx, θy; unit=unit)
end

function get_jacobian(model::ModelConfig, chains::Array{Float64, 3}, chi2::Matrix{Float64}, θx::T, θy::T; unit::Symbol=:RA_DEC) where T <: Union{ROA, Vector{Int64}}
   # Check if the input coordinates are of the same size
   if size(θx) != size(θy)
      throw(ArgumentError("Input coordinates must be of the same size."))
   end
