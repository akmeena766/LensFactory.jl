...
# --------------------------------------------------------------------------------------------------
# Various source profiles to export
# --------------------------------------------------------------------------------------------------
export disk
export gaussian
export sersic
...
"""
function disk(θ_x::Matrix{<:RV}, θ_y::Matrix{<:RV}, θ_r::RV, β::NTuple{2, RV}; A::RV=1.0)::Matrix{<:RV}
   ...
   @inbounds for j in ax2
      @inbounds for i in ax1
         dx = θ_x[i, j] - β[1]
         dy = θ_y[i, j] - β[2]
         src[i, j] = (dx^2 + dy^2 <= θ_r^2) ? A * 1.0 : 0.0
      end
   end
   return src
end
...
- `θ_x::Matrix{<:RV}`: x-grid (in ``\rm \\mathbf{arcseconds}``).
- `θ_y::Matrix{<:RV}`: y-grid (in ``\rm \\mathbf{arcseconds}``).
- `σ_x::RV`: Standard deviation along the x-axis (in ``\rm \\mathbf{arcseconds}``).
- `σ_y::RV`: Standard deviation along the y-axis (in ``\rm \\mathbf{arcseconds}``).
- `β::NTuple{2, RV}`: Center of the Gaussian (in ``\rm \\mathbf{arcseconds}``).

# Keyword Arguments
- `A::RV`: Amplitude of the Gaussian (in ``\rm \\mathbf{arcseconds}``).

# Returns
- `src::Matrix{<:RV}`: Source profile on a grid defined by ``[θ_x, θ_y]``.
"""
function gaussian(θ_x::Matrix{<:RV}, θ_y::Matrix{<:RV}, σ_x::RV, σ_y::RV, β::NTuple{2, RV}; A::RV=1.0)::Matrix{<:RV}
   # Initialize an empty source grid
   src = zero(θ_x)
...
- `src::Matrix{<:RV}`: Source profile on a grid defined by ``[θ_x, θ_y]``.
"""
function sersic(θ_x::Matrix{<:RV}, θ_y::Matrix{<:RV}, n::RV, θ_e::RV, β::NTuple{2, RV}; A::RV=1.0)::Matrix{<:RV}
   # Initialize an empty source grid
   src = zero(θ_x)
   bn::Float64 = b_n(n)

   # Normalization factor
   amplitude::Float64 = A * bn^(2n) / (π * θ_e^2 * gamma(2.0 * n + 1.0))
   ...
   @inbounds for j in ax2
      @inbounds for i in ax1
         dx = θ_x[i, j] - β[1]
         dy = θ_y[i, j] - β[2]
         src[i, j] = amplitude * exp(-bn * (√(dx^2 + dy^2) / θ_e)^(1/n))
      end
   end
   return src
end
