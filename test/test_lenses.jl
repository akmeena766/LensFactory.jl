@testset "Lenses" begin
   ...
   @testset "Σ_cr" begin
      ...
      @test Σcr * (DIST_PC^2 / MASS_SUN) ≈ Lenses.get_critical_density(D_d=1.0, D_ds=1.0, D_s=1; unit=:msun_pc2)

      # Unit conversion: kg/m^2 --> msun/arcsec^2
      @test Σcr * (D_d^2 * ANGLE_ARCSEC^2 / MASS_SUN) ≈ Lenses.get_critical_density(D_d=1.0, D_ds=1.0, D_s=1; unit=:msun_arcsec2)
